...
reactimateNet senseI sense actuate sf netin combine netout = do
  MSF.reactimateB
    $   (senseSF senseI sense &&& (arrM $ \() -> netin)) >>> arr combine
    >>> sfIO sf
    >>> (actuateSF actuate)
...
switch' :: Monad m =>
                 MSF m a (b, Event c) -> (c -> MSF m a b) -> MSF m a b
