...
gameSF :: Monad m => SF (PlayerEnv m) GameInput GameState
gameSF = arr directionInput >>> paddleSF >>> arr GameState
...
ballSF :: Monad m => SF (BallEnv m) (Maybe Direction) BallSettings
ballSF = proc dir -> do
  c <- morphS bsToPs colorSF -< undefined
  (p, v) <- morphS bsToPs moveSF -< dir
  returnA -< BallSettings p v c
  where
